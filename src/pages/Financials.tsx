...
const Financials = () => {
  ...
  useEffect(() => {
    fetchAllData();
  }, [fetchAllData]); // Re-fetch when date range changes or on initial mount
  ...
  useEffect(() => {
    ...
    allTransactions.forEach(transaction => {
      ...
      if (account.type === 'Asset' || account.type === 'Expense') {
        ...
        if (transaction.type === 'income' || transaction.type === 'debt') { // Money coming in (e.g., sales, loan received)
          accountBalances[account.name] += amount;
        } else if (transaction.type === 'expense') { // Money going out (e.g., paying liability)
          accountBalances[account.name] -= amount;
        }
      }
    });
